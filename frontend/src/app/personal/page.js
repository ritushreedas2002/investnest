...
const Dashboard = () => {
  const [financialData, setFinancialData] = useState({ totalIncome: 0, totalExpense: 0 });
  const email =typeof window !== "undefined" ? localStorage.getItem("email") : null;
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/api/analysis/yearly/total?email=${email}`);
        setFinancialData({
          ...
          <div className="w-[99%] h-[400px] p-4 mb-4 bg-gray-600 rounded-2xl">
            ...
            <div className="flex flex-wrap">
              <div className="w-1/2 pr-1.5 py-1.5">
                <TransactionCard name="Total Income" amount={`$${financialData.totalIncome.toFixed(2)}`} />
              </div>
              <div className="w-1/2 py-1.5">
                <TransactionCard name="Total Expense" amount={`$${financialData.totalExpense.toFixed(2)}`} />
              </div>
            </div>
            <BarGraphComponent />
          </div>
