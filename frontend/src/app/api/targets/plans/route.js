...
export async function GET(request){
    try{
        const email = request.nextUrl.searchParams.get('email'); // Correct way to get query parameters in Next.js middleware


        if (!email) {
            return new NextResponse(JSON.stringify({ msg:"Email parameter is required" }), { status: 400 });
        }
        ...
        return new NextResponse(JSON.stringify({ plans: user.Plans }), { status: 200 });
    }catch(error){
...
export async function POST(request) {
    try {
        ...
        if (user) {
            ...
            await user.save();
            return new NextResponse(JSON.stringify({ message: "Plan added to existing user" }), { status: 200 });
        } else {
            ...
            await newUser.save();
            return new NextResponse(JSON.stringify({ message: "New user created with plan" }), { status: 201 });
        }
    } catch (error) {
...
export async function PUT(request) {
    try {
        ...
        const user = await PlanningModel.findOne({ userId: email, 'Plans._id': new mongoose.Types.ObjectId(id) });
            ...
            if (plan.GoalTarget > 0) {
                plan.Achieved = parseFloat(((plan.CurrentAmount / plan.GoalTarget) * 100).toFixed(2));
            }

            // Save the user document
            await user.save();
            return new NextResponse(JSON.stringify({ mag: "Updated Successfully" }), { status: 200 });
    } catch (error) {
        console.error('Error processing POST request:', error);
        return new NextResponse(JSON.stringify({ error: error.message }), { status: 500 });
    }
...
export async function DELETE(request) {
    try {
        ...
        if (result.modifiedCount === 0) {
            return NextResponse.json({message:"No plan found with the given ID or user does not exist." }, { status: 200 });
        } else {
