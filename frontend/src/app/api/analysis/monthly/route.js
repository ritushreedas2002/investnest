...
export async function GET(request) {
    try {
        ...
        if (!userTransactions || !userTransactions.years || !userTransactions.years.get(currentYear)) {
            return new NextResponse(JSON.stringify({ message: "No data for the year" }), { status: 404 });
        }
    
        const monthlyData = userTransactions.years.get(currentYear).months.get(currentMonth);
        if (!monthlyData) {
            return new NextResponse(JSON.stringify({ message: "No data for the month" }), { status: 404 });
        }
