...
export default function VerifyEmailPage() {
    ...
    const verifyUserEmail = async () => {
        try {
            const response = await axios.post('/api/users/verifyemail', { token, emailType: "VERIFY" });
            setVerified(true);
        ...
        <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 p-4">
            ...
            {verified && (
                <div className="text-lg text-green-500">
                    <p>{emailType === "VERIFY" ? "Email Verified" : "Password Reset Successfully"}</p>
                    <Link href="/login" passHref className="text-indigo-600 hover:text-indigo-900">Login
                    </Link>
                </div>
            )}
            {error && (
