...
const ChartComponent = ({ data, currency, type }) => {
    if (!data || !Array.isArray(data) || data.length === 0) {
        return (
            <div className="w-full h-full flex justify-center items-center">
                <div className="w-3 h-3 border-4 border-cyan-700 rounded-full border-b-gray-200 animate-spin" role="status" />
                <span className="ml-2">Loading...</span>
            </div>
        );
    }
    ...
    const dates = data.map((price) => new Date(price[0]).toLocaleDateString());
    const prices = data.map(price => isNaN(parseFloat(price[1])) ? '0.00' : parseFloat(price[1]).toFixed(2));
