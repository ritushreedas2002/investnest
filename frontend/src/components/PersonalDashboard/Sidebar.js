...
const Sidebar = () => {
  ...
  return (
    <div className="sidebar__wrapper fixed z-50 bg-blue-950 h-full">
      <button className="btn absolute right-0 top-20 translate-x-1/2 bg-black border-0 z-50 w-6 h-6 rounded-full flex justify-center items-center cursor-pointer text-lg" onClick={toggleSidebarcollapse}>
        {isCollapsed ? <MdKeyboardArrowRight className="text-white" /> : <MdKeyboardArrowLeft className="text-white" />}
        {console.log(isCollapsed)}
      </button>
      <aside className="sidebar w-52 h-full p-4 transition-all duration-400 ease-linear" data-collapse={isCollapsed}>
        <div className="sidebar__top w-max flex items-center gap-4  mb-2 ">
          <Image
            width={80}
            height={80}
            className="sidebar__logo object-cover "
            ...
            alt="logo"
          />
          {/* <p className="sidebar__logo-name text-lg font-semibold text-white">Finnest</p> */}
        </div>
        <ul className="sidebar__list list-none">
          {sidebarItems.map(({ name, href, icon: Icon }) => {
            return (
              <li className="sidebar__item" key={name}>
                <Link
                  className="sidebar__link text-base no-underline text-white py-3 px-4 flex bg-transparent mb-4 rounded-lg hover:bg-cyan-700 hover:backdrop-blur-sm"
                  href={href}
                >
                  <span className="sidebar__icon text-2xl inline-block text-cyan-300">
                    <Icon />
                  </span>
                  <span className="sidebar__name  text-cyan-300 ml-2">{name}</span>
                </Link>
              </li>
            );
          })}
